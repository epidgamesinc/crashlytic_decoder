...
async function processPage() {
  ...
  if (
    window.location.href.includes('trickcal-revive/crashlytics/app') === false
  ) {
    return;
  }
  ...
  if (window.location.href !== lastUrl) {
    ...
    if(isValidStatus(pageVersion, data)){
      await processNewElements(pageVersion, data);
    }
...
function isValidStatus(pageVersion, data) {
  return !(
    pageVersion?.os == null ||
    data == null ||
...
// 공통 상태바 생성 함수
function setLoadingStatusBar() {
  // 기존 상태바 제거
  const existingBar = document.getElementById('floating-status-bar');
  ...
  statusBar.style.boxShadow = '0 2px 10px rgba(0,0,0,0.05)';
  statusBar.style.color = '#5a5a5a';
  statusBar.style.fontFamily = 'Arial, sans-serif';
  statusBar.style.fontSize = '14px';
  statusBar.style.backdropFilter = 'blur(4px)';
  statusBar.style.transition = 'all 0.4s cubic-bezier(0.16, 1, 0.3, 1)';
  statusBar.style.display = 'flex';
  statusBar.style.alignItems = 'center';
  statusBar.style.justifyContent = 'center';
  statusBar.style.gap = '8px';

  statusBar.style.backgroundColor = 'rgba(220, 237, 200, 0.9)'; // 파스텔 그린
  statusBar.style.borderBottom = '1px solid rgba(180, 207, 180, 0.5)';
...
function displayStatus(pageVersion, data) {
  ...
  // 유효성 검사
  const isValid = isValidStatus(pageVersion, data);
  ...
  if (isValid) {
    ...
    statusBar.textContent = '☁️ 현재 페이지';
    ...
    if(data == null || data[pageVersion.os] == null){
      statusBar.textContent += ' 없음';
    }
    else{
      statusBar.textContent += ` v${data[pageVersion.os].version}`;
    }
...
async function getCurrentPageOSVersion() {
  ...
  if (
    ...
    if (
      document.location.href.includes('trickcal-revive/crashlytics/app/ios:')
    ) {
      os = 'ios';
      version = document
        .querySelector('.version-title')
        .innerHTML.match(/\d+/)[0];
    } else if (
      document.location.href.includes(
        'trickcal-revive/crashlytics/app/android:'
      )
    ) {
      os = 'aos';

      let nodes = [
        ...document.querySelectorAll('.mat-mdc-tooltip-trigger.version-range'),
      ];
  ...
  if (
    document.querySelectorAll('.gmp-icons.platform-icon.ng-star-inserted')
      .length > 0 &&
    document
      .querySelectorAll('.gmp-icons.platform-icon.ng-star-inserted')[0]
      .innerHTML.includes('ios')
  ) {
    os = 'ios';
    var obj = document
      .querySelector('.session-build-version.header-item')
      ?.querySelector('.header-item-text.ng-star-inserted')?.innerHTML;
...
async function processSummarizeElement(parsedMapping) {
  const cells = document.querySelectorAll(
    '.caption-table-cell.ng-star-inserted'
  );
  // 각 요소에 스타일 적용
  cells.forEach((cell) => {
...
async function processAOSElement(parsedMapping) {
  ...
  cells.forEach((cell) => {
    ...
    if (wrappers != null) {
      const { className, methodName } = parseAndroidStack(
        wrappers.textContent.trim()
      );
...
function waitForAjaxComplete() {
  ...
  return new Promise((resolve) => {
    const interval = setInterval(() => {
      // 지정된 요소가 존재하는지 확인
      if (
        document.querySelectorAll('.caption-table-cell.ng-star-inserted')
          .length > 0
      ) {
      ...
      if (
        document.querySelectorAll('.library-cell.ng-star-inserted').length > 0
      ) {
        clearInterval(interval); // 요소가 나타났으면 interval 멈추기
        resolve();
      }
      ...
      if (
        document.querySelectorAll('.stack-frame.ng-star-inserted').length > 0
      ) {
        clearInterval(interval); // 요소가 나타났으면 interval 멈추기
        resolve();
      }
...
function partialEncode(content) {
  ...
  for (var i = 0; i < compressed.length; i += chunkSize) {
    ...
    var binaryString = Array.from(chunk)
...
function partialDecode(encodedString, originalChunkSize = 1020) {
  ...
  while (pos < encodedString.length) {
    ...
    var chunkLength = Math.min(
      (originalChunkSize * 4) / 3,
      encodedString.length - pos
    );
...
function addTranslatedMessage(wrappers, msg) {
  ...
  const existingMessage = wrappers.parentNode.querySelector('.translated-message');
  if (existingMessage) {
